...
pub fn run_watch(
    ...
    if !config.input.is_dir() {
        eprintln!("error: input directory '{}' not found", config.input.display());
        return EXIT_PATH;
    }
    ...
    if let Some(rd) = &config.report {
        if let Err(e) = std::fs::create_dir_all(rd) {
            eprintln!("error: cannot create report directory '{}': {e}", rd.display());
            return EXIT_ERROR;
        }
    }
...
fn process_one(
    ...
    let asset = match build_asset(path) {
        Ok(a) => a,
        Err(_) => {
            eprintln!("watch: could not fingerprint '{}'; leaving in place", path.display());
            return;
        }
    };
    let run = match engine.check(&asset, level) {
        Ok(r) => r,
        Err(e) => {
            eprintln!("watch: probe failed for '{}': {e}; leaving in place", path.display());
            return;
        }
    };
    ...
    if let Some(rd) = &config.report {
        ...
        if let Err(e) = write_reports(&run, profile, Some(&rd.join(name)), None, None, None) {
            eprintln!("watch: could not write report for '{}': {e}", path.display());
        }
    }
...
mod tests {
    ...
    fn new_file_event_matches_create() {
        ...
        assert!(is_new_file_event(&EventKind::Modify(ModifyKind::Name(RenameMode::To))));
